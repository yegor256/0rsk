  ...
  def query(id, query)
    ...
    Rsk::Query.new(
      ...
      [
        ...
        '  COALESCE((',
        '    SELECT JSONB_AGG(JSONB_BUILD_OBJECT(\'id\', apart.id, \'text\', apart.text || \' (\' || a.schedule || \')\'))',
        '    FROM plan a JOIN part AS apart ON a.id = apart.id',
        '    WHERE a.part = t.cause OR a.part = t.risk OR a.part = t.effect',
        '  ), \'[]\'::JSONB) AS plans',
        'FROM triple t',
